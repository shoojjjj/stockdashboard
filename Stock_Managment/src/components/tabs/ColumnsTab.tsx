        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          {fullscreen && onClose && (
            <button
              type="button"
              onClick={onClose}
              className="text-xs px-2.5 py-1 rounded-lg bg-slate-800 text-white hover:bg-slate-700"
            >
              닫기 ✕
            </button>
          )}
  ...
  return (
    <div className="space-y-4">
      <p className="text-slate-600 text-sm">
        최신순 정렬 · 제목 클릭 시 본문 표시 · PC에서는 <strong>넓게 읽기</strong>로 전체 화면
      </p>
      ...
      <div className="grid lg:grid-cols-[minmax(220px,280px)_1fr] gap-4 min-h-[420px]">
        <div className="bg-white rounded-2xl border p-4 flex flex-col lg:max-h-[calc(100vh-240px)]">
          ...
          />
          <ul className="flex-1 overflow-y-auto divide-y divide-slate-100 min-h-[200px] lg:min-h-0">
            {filtered.map((a) => (
              <li key={a.id}>
                <button
                  type="button"
                  ...
                  disabled={loading}
                  className={`w-full text-left text-sm px-2 py-2 hover:bg-indigo-50 transition flex items-start justify-between gap-2 ${
        ...
        <div className="bg-white rounded-2xl border p-4 lg:p-5 overflow-y-auto lg:min-h-[480px] lg:max-h-[calc(100vh-240px)]">
      ...
      {fullscreen && selected && (
        <div
          className="fixed inset-0 z-50 bg-slate-50/98 backdrop-blur-sm"
          role="dialog"
          aria-modal="true"

...
def parse_portfolio(text: str) -> tuple[set[str], int, list[str]]:
    held: set[str] = set()
    notes: list[str] = []
    pending = text.count("_(입력)_")
    for row in parse_md_table(section(text, "보유 종목")):
        ...
        if ticker and not ticker.startswith("_"):
            ...
            if len(row) > 5 and row[5]:
                notes.append(f"{ticker}: {row[5]}")
    cash_sec = section(text, "현금·여력")
    if "부족" in cash_sec:
        notes.append("현금 여력 부족 가능")
    elif "_(있음/부족)_" in cash_sec:
        notes.append("현금 여력 미입력")
    return held, pending, notes
...
def portfolio_col(ticker: str, held: set[str], notes: list[str]) -> str:
    ...
    if held_any:
        if "PLTR" in ticker.upper() or "추가 보류" in note_blob:
            return "이미 보유 · 추가 보류"
        if "CRWV" in ticker.upper():
            return "보유 · 물타기 금지"
        return "이미 보유"
    if pending_placeholder(note_blob):
        return "여력 확인 필요"
    return "미보유"


def pending_placeholder(_: str) -> bool:
    return True  # portfolio often has _(입력)_
...
def mediate(
    ...
    *,
    ...
    if "추가 금지" in pf or "물타기 금지" in pf:
        return "홀딩, 추가 금지"
    ...
    if conditional:
        ...
        return "조건부 대기"
    if "이미 보유" in pf:
        if gate.startswith("G2") and signal in ("SM-S3", "SM-S2"):
            return "눌림 시 추가"
        return "홀딩"
...
def build_meeting(
    ...
    do_today: list[str] = []
    dont_today = dont[:4] if dont else []
    if not any("눌림" in r[4] for r in rows):
        do_today.append("_(없음 — 조건부 대기 우세)_")
    if held & {"MU"} or any("DRAM" in r[0] for r in rows):
        do_today.append("보유 MU / DRAM ETF: 방향 유지, 추가는 눌림 시")

    protect: list[str] = []
    for h in hold:
        protect.append(h.replace(":", " —"))
    if "PLTR" in held:
        protect.append("PLTR: 홀딩, 추가 금지")
    if "CRWV" in held:
        protect.append("CRWV: 홀딩, 물타기 금지")

    reduce_lines = [f"{r}: 정리 후보 유지 (신호 약함)" for r in reduce] or ["_(없음)_"]
    ...
    if any("ARM" in r[0] for r in rows):
        conflicts.append("ARM: 신호는 강하나 급등·과열 → 2차 대기로 조정")

    user_checks = [
        "실제 계좌 현금 여력 확인 후 NVDA/MSFT 1차 진입 여부 결정",
        "SLV/GDX 정리 실행 여부",
    ]
    if pending > 0:
...
def main() -> int:
    ...
    held, pending, pf_notes = parse_portfolio(portfolio_text)
